...
using PrettyTimetable.Services.Asc.Timetable.AscClasses;
using System.Runtime.Serialization;
using System.Xml;
using System.Xml.Serialization;
using static PrettyTimetable.Services.Asc.Timetable.StaticDeterminers;

namespace PrettyTimetable.Services.Asc.Timetable
{
    internal class Converter(TimetableContext timetableContext)
    {
        public List<Core.Entities.Timetable.Cards.Info.Room> Cabinets { get; private set; } = [];
        public List<Core.Entities.Timetable.Cards.Info.Teacher> Teachers { get; private set; } = [];
        public List<Core.Entities.Timetable.Cards.Info.LessonTime> LessonTimes { get; private set; } = [];
        public List<Core.Entities.Timetable.Cards.Info.Subject> Subjects { get; private set; } = [];
        public List<Core.Entities.Timetable.Cards.StableCard> StableCards { get; private set; } = [];
        public List<Core.Entities.Timetable.StableTimetable> StableTimetables { get; private set; } = [];
        public List<Core.Entities.Timetable.Group> Groups { get; private set; } = [];

        private AscTimetable _ascTimetable = null!;
        ...
        public async Task ConvertAndSaveAsync(XmlReader xmlReader, CancellationToken cancellationToken = default)
        {
            ...
            _ascTimetable = serializer.Deserialize(xmlReader) as AscTimetable ?? throw new SerializationException();
        ...
        private void FillDataForCards()
        {
            ...
            Groups = _ascTimetable.Classes.Class.Select(e => new Core.Entities.Timetable.Group()
            { Id = default, AscId = e.Id, Name = e.Name, ModifiedAt = DateTime.UtcNow }).ToList();

            Teachers = _ascTimetable.Teachers.Teacher.Select(e => new Core.Entities.Timetable.Cards.Info.Teacher()
            {
                Id = default,
                AscId = e.Id,
                Lastname = e.Lastname,
            ...
            Subjects = _ascTimetable.Subjects.Subject.Select(e => new Core.Entities.Timetable.Cards.Info.Subject()
            { Id = default, AscId = e.Id, Name = e.Name, ModifiedAt = DateTime.UtcNow }).ToList();

            LessonTimes = _ascTimetable.Periods.Period.Select(e => new Core.Entities.Timetable.Cards.Info.LessonTime()
            ...
            Cabinets = _ascTimetable.Classrooms.Classroom.Select(e => new Core.Entities.Timetable.Cards.Info.Room()
            {
                Id = default,
                AscId = e.Id,
                Number = e.Short,
        ...
        {
            List<Core.Entities.Timetable.StableTimetable> stableTimetables = [];

            foreach (var group in Groups)
            {
                var stableCardsOfCurrentGroup = new List<Core.Entities.Timetable.Cards.StableCard>();
                var lessonsOfCurrentGroup = _ascTimetable.Lessons.Lesson.Where(e => e.Classids == group.AscId).ToList();
                var ascCardsOfCurrentGroup = new List<Card>();
                ...
                foreach (var card in ascCardsOfCurrentGroup)
                {
                    ...
                    Core.Entities.Timetable.Cards.Info.Teacher teacher = Teachers.Single(e => e.AscId == lesson.Teacherids);
                    Core.Entities.Timetable.Cards.Info.Subject subject = Subjects.Single(e => e.AscId == lesson.Subjectid);
                    Core.Entities.Timetable.Cards.Info.LessonTime lessonTime = LessonTimes.Single(e => e.Number == int.Parse(card.Period));
                    Core.Entities.Timetable.Cards.Info.Room cabinet = DetermineCabinets(Cabinets, lesson.Classroomids);

                    Core.Entities.Timetable.Cards.Info.SubGroup subGroup = DetermineSubgroup(_ascTimetable.Groups.Group.Single(e => e.Id == lesson.Groupids).Name);
                    switch (DetermineWeekEvenness(card.Weeks))
                    {
                        case WeekEvenness.Both:
                            stableCardsOfCurrentGroup.Add(new Core.Entities.Timetable.Cards.StableCard()
                            {
                                Id = default,
                ...
                var currentStableTimetable = new Core.Entities.Timetable.StableTimetable() { Id = default, Cards = stableCardsOfCurrentGroup, GroupId = group.Id, ModifiedAt = DateTime.UtcNow };
