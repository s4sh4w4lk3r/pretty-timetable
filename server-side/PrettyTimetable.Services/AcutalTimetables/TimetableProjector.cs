...
namespace PrettyTimetable.Services.AcutalTimetables
{
    internal class TimetableProjector(TimetableContext timetableContext)
    {
        ...
        private static ServiceResult<int> CheckDates(IEnumerable<DateOnly> dates)
        {
            ...
            {
                return ServiceResult.Fail("Массив с датами пустой или нулл.", WEEK_NUMBER_IF_ERROR);
            }
            ...
            {
                {
                    ...
                    {
                        return ServiceResult.Fail("Передан массив дат, которые указывают на разные недели", WEEK_NUMBER_IF_ERROR);
                    }
                }
            }

            return ServiceResult.Ok("Ок", weekNumber);
        }
