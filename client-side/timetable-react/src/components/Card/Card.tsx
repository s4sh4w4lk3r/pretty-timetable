...
function defineHintsRequired(
    lessonStartsAt: string,
    lessonEndsAt: string,
    dateFromCard: string
): { isNow: boolean; isPending: boolean } {
    const startDate = new Date(
        new Date().setHours(
            Number.parseInt(lessonStartsAt.split(":")[0]),
            Number.parseInt(lessonStartsAt.split(":")[1]),
            0
        )
    );
    const endDate = new Date(
        new Date().setHours(
            Number.parseInt(lessonEndsAt.split(":")[0]),
            Number.parseInt(lessonEndsAt.split(":")[1]),
            0
        )
    );
    const dateOk =
        new Date(Date.parse(dateFromCard)).getUTCDate() ===
        new Date().getUTCDate();
    ...
    const isPending =
        dateOk &&
        timeFromNowToLessonStart.includes("minutes") &&
        Number.parseInt(timeFromNowToLessonStart) <= 30;
...
export default function Card(props: Props) {
    ...
    const { isPending, isNow } = defineHintsRequired(
        startsAt,
        endsAt,
        props.card.date
    );
    const cardClass = defineCardClass(
        isNow,
        isPending,
        isCanceled,
        isModified || isMoved
