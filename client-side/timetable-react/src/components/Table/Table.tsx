...
export default function Table(props: Props) {
    ...
    const dates = [...new Set(cards.map(c => c.date))].map(d => new Date(d)).sort((date1, date2) => (date1.getTime() > date2.getTime() ? 1 : -1));
    ...
    return (
        <>
            <div className={styles.table}>
                ...
                <div className={styles.cardContainers}>{cardContainersElement}</div>
            </div>
